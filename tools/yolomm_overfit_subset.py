"""Create a true same-split subset overfit run for existing YOLOMM baselines."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLOMM true subset overfit")
    parser.add_argument("--model", default="ultralytics/cfg/models/mm/v8/yolov8-mm-early.yaml")
    ...
    parser.add_argument("--project", default="runs/yolomm")
...
def main():
    ...
    temp_dir = (Path("runs") / "yolomm_subset").resolve()
    ...
    model = YOLOMM(args.model, task="detect", ch=6, verbose=True)
    model.train(
