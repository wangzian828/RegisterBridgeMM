...
def plot_images(
    ...
    Args:
        ...
        kpts: Keypoints for each detection. Shape: (num_detections, 51) for COCO format.
    ...
    Note:
        ...
        convert tensor inputs to numpy arrays for processing. Compatible with YOLOMM multi-modal training.
    """
    ...
    # Handle shape adjustments
    if cls.ndim == 2:
        cls = cls.squeeze(1)  # squeeze if shape is (n, 1)

    # Handle multi-spectral images (crop to first 3 channels for visualization)
    if images.shape[1] > 3:
...
def plot_results(
    ...
    # 读取第一个文件获取列信息，用于动态确定绘图布局
    first_data = pd.read_csv(files[0])
    columns = [x.strip() for x in first_data.columns]
    ...
    if classify:
        ...
        # Pose 模式：按列名动态构建索引，兼容有/无 val loss 的 CSV 格式
        pose_col_names = [
            # 训练损失
        ...
        index = []
        valid_col_names = []
        for col_name in pose_col_names:
            if col_name in columns:
                index.append(columns.index(col_name))
                valid_col_names.append(col_name)
        # 计算子图布局
        n_plots = len(index)
        if n_plots == 0:
            # 回退到默认索引（跳过越界的）
            default_index = [2, 3, 4, 5, 6, 7, 8, 11, 12, 15, 16, 17, 18, 19, 9, 10, 13, 14]
            index = [i for i in default_index if i < num_cols]
            n_plots = len(index)
        n_cols = min(9, max(5, (n_plots + 1) // 2))
        n_rows = (n_plots + n_cols - 1) // n_cols
        fig, ax = plt.subplots(n_rows, n_cols, figsize=(n_cols * 2.3, n_rows * 3), tight_layout=True)
    else:
...
def output_to_target(output, max_det=300):
    ...
    Args:
        output (list): Model output predictions
        max_det (int): Maximum number of detections per image

    Returns:
        tuple: (batch_ids, class_ids, boxes, confidences) for plotting
    """
    targets = []
    for i, o in enumerate(output):
        ...
        try:
            ...
            if isinstance(o, dict):
                # 处理标准的postprocess输出格式：{'bboxes', 'conf', 'cls', 'extra'}
                if 'bboxes' in o and 'conf' in o and 'cls' in o:
                    bboxes = o['bboxes']  # [N, 4]
                    conf = o['conf'].unsqueeze(1) if o['conf'].dim() == 1 else o['conf']  # [N, 1]
                    cls = o['cls'].unsqueeze(1) if o['cls'].dim() == 1 else o['cls']  # [N, 1]
                    # 重新组合为标准格式 [N, 6]: [x1, y1, x2, y2, conf, cls]
                    o = torch.cat([bboxes, conf, cls], dim=1)
                # 其他常见格式
                elif 'pred' in o:
                    o = o['pred']
                elif 'det' in o:
                    ...
                    # 如果是分离的boxes和scores，需要重新组合
                    boxes = o['boxes']
                    scores = o.get('scores', torch.ones(boxes.shape[0], 1))
                    labels = o.get('labels', torch.zeros(boxes.shape[0], 1))
                    o = torch.cat([boxes, scores, labels], dim=1)
                else:
                    # 尝试使用第一个张量值
                    tensor_values = [v for v in o.values() if torch.is_tensor(v)]
                    if tensor_values:
                        o = tensor_values[0]
                    else:
                        raise ValueError(f"无法从字典中提取张量数据: {list(o.keys())}")

            # 确保o是张量
            if not torch.is_tensor(o):
                raise TypeError(f"预期张量，但得到 {type(o)}")

            # 确保张量有足够的维度和特征
            if o.dim() < 2 or o.shape[1] < 6:
                raise ValueError(f"张量形状不正确: {o.shape}，期望 [N, >=6]")
            ...
            # 详细的错误信息
            print(f"output_to_target错误 - 批次{i}: {e}")
            print(f"o的类型: {type(o)}")
            if hasattr(o, 'shape'):
                print(f"o的形状: {o.shape}")
            elif isinstance(o, dict):
                print(f"o的键: {list(o.keys())}")
                for k, v in o.items():
    ...
    if not targets:
        # 如果没有有效的目标，返回空结果
        return np.array([]), np.array([]), np.array([]).reshape(0, 4), np.array([])
