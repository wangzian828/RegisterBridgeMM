...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int = None, tal_stride: list = None):  # model must be de-paralleled
        """
    ...
    def __call__(self, preds: Any, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with autocast(enabled=False):
            ...
            # Cls loss (BCE)
            loss[1] = self.bce(pred_scores_f, target_scores.to(torch.float32)).sum() / target_scores_sum
