...
class Model(torch.nn.Module):
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        Args:
            source: 图像源（向后兼容参数，等价于rgb_source）
            stream: 是否流式返回结果
            rgb_source: RGB图像源（keyword-only）
                - 单图: '/path/to/rgb.jpg'
                - 批量: ['/path/rgb1.jpg', '/path/rgb2.jpg']
            x_source: X模态图像源（keyword-only，thermal/depth/ir等）
                - 单图: '/path/to/thermal.jpg'
                - 批量: ['/path/thermal1.jpg', '/path/thermal2.jpg']
            strict_match (bool): 批量推理时的匹配策略（keyword-only）
                - True: 严格模式，两个目录/列表必须完全匹配，否则报错
                - False: 交集模式，只处理能匹配的，跳过不匹配的并警告
                - 单对图片输入时此参数被忽略
            predictor: 自定义预测器实例（可选）
            **kwargs: 额外配置参数
                - conf (float): 置信度阈值（默认0.25，验证时为0.001）
                - iou (float): NMS IOU阈值（默认0.45）
                - max_det (int): 最大检测框数量（默认300）
                - save (bool): 是否保存结果
                - 其他参数...
        ...
        Examples:
            ...
            ...     rgb_source='/path/to/rgb.jpg',
            ...     x_source='/path/to/thermal.jpg'
            ... )

            # 使用自定义阈值（解决验证高精度但推理零检测的问题）
            >>> results = model.predict(
            ...     rgb_source='/path/to/rgb.jpg',
            ...     x_source='/path/to/thermal.jpg',
            ...     conf=0.001  # 使用与验证相同的阈值
            ... )

            # 批量推理
            >>> results = model.predict(
            ...     rgb_source=['/path/rgb1.jpg', '/path/rgb2.jpg'],
            ...     x_source=['/path/thermal1.jpg', '/path/thermal2.jpg']
            ... )

            # 向后兼容（标准YOLO单模态）
            >>> model = YOLO("yolo11n.pt")
            >>> results = model.predict(source='/path/to/image.jpg')
            >>> results = model.predict('/path/to/image.jpg')  # 位置参数

        Raises:
            ValueError: 当所有源参数都缺失时
        """
        # 向后兼容逻辑：source 参数映射到 rgb_source
        if source is not None and rgb_source is None:
            rgb_source = source

        # 参数验证（支持单模态推理）
        if rgb_source is None and x_source is None:
            raise ValueError(
                "至少需要提供 source、rgb_source 或 x_source 参数之一。\n"
                "示例:\n"
                "  标准YOLO: model.predict(source='/path/image.jpg')  # 向后兼容\n"
                "  双模态: model.predict(rgb_source='/path/rgb.jpg', x_source='/path/thermal.jpg')\n"
                "  单RGB: model.predict(rgb_source='/path/rgb.jpg', x_source=None)\n"
                "  单X:   model.predict(rgb_source=None, x_source='/path/thermal.jpg')"
            )
        ...
        # 传递 strict_match 参数（仅批量推理时生效）
        args["strict_match"] = strict_match
        prompts = args.pop("prompts", None)

        if not self.predictor:
            self.predictor = (predictor or self._smart_load("predictor"))(overrides=args, _callbacks=self.callbacks)
            # verbose: 控制常规日志输出（模型加载、推理进度等）
            # debug: 控制DEBUG调试日志输出（默认False）
            # 从 predictor.args 读取（已通过 overrides=args 设置）
            self.predictor.setup_model(
                model=self.model,
                verbose=self.predictor.args.verbose,
                debug=self.predictor.args.debug
            )
        else:
        ...
        # 传递显式参数给 predictor
        # 检查 predictor 是否支持多模态API
        # 匹配模式：
        #   1. 有 'predictor' 属性（YOLOMMPredictor 适配器模式）
        #   2. 类名包含 'MM'（如 YOLOMMPredictor, RTDETRMMPredictor）
        #   3. 类名以 'MultiModal' 开头（如 MultiModalSegmentationPredictor）
        predictor_class_name = self.predictor.__class__.__name__
        is_multimodal_predictor = (
            hasattr(self.predictor, 'predictor') or
            'MM' in predictor_class_name or
            predictor_class_name.startswith('MultiModal')
        )

        if is_cli:
            # CLI 模式
            if is_multimodal_predictor:
                return self.predictor.predict_cli(rgb_source=rgb_source, x_source=x_source, strict_match=strict_match)
            else:
                # 标准predictor只接受source参数
                return self.predictor.predict_cli(source=rgb_source)
        else:
            # 非CLI模式
            if is_multimodal_predictor:
                return self.predictor(rgb_source=rgb_source, x_source=x_source, stream=stream, strict_match=strict_match)
            else:
                # 标准predictor只接受source参数
                return self.predictor(source=rgb_source, stream=stream)
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        The method now supports both standard validation (mode='val') and COCO validation (mode='cocoval') modes.
        COCO validation provides more detailed metrics including 12 standard COCO evaluation metrics.

        Args:
            validator (ultralytics.engine.validator.BaseValidator, optional): An instance of a custom validator class
                for validating the model.
            **kwargs (Any): Arbitrary keyword arguments for customizing the validation process. Special arguments:
                mode (str): Validation mode, 'val' for standard validation or 'cocoval' for COCO evaluation.
                    Defaults to 'val' for backward compatibility.

        Returns:
            (ultralytics.utils.metrics.DetMetrics): Validation metrics obtained from the validation process.
                For COCO validation, returns a dict with detailed COCO metrics.
        ...
        Examples:
            ...
            >>> # COCO evaluation
            >>> coco_results = model.val(data="coco8.yaml", mode="cocoval")
            >>> print(coco_results['metrics/coco/AP'])  # Print COCO mAP@0.5:0.95
        """
        # 检测验证模式：标准验证 vs COCO验证
        validation_mode = kwargs.get("mode", "val")
        
        # 为COCO验证模式设置不同的默认参数
        if validation_mode == "cocoval":
            custom = {
                "rect": True,  # 矩形推理以提高效率
                "save_json": kwargs.get("save_json", True),  # COCO验证默认保存JSON结果
                "plots": kwargs.get("plots", True),  # COCO验证默认生成可视化图表
            }
        else:
            custom = {"rect": True}  # 标准验证的方法默认值
        
        # 构建验证参数，确保mode参数被正确设置
        args = {**self.overrides, **custom, **kwargs}
        if "mode" not in args:
            args["mode"] = "val"  # 默认为标准验证模式
        ...
        **kwargs: Any,
        ...
        if not args.get("resume"):  # manually set model only if not resuming
            ...
            if model_scale:
                if self.ckpt:
                    raise ValueError(
                        "model_scale/scale can only be used when building a model from YAML. "
                        "Please train from a YAML model (not a .pt checkpoint) or remove model_scale/scale."
                    )
                if not isinstance(cfg_for_build, dict):
                    raise TypeError("Internal error: expected model YAML cfg as a dict when applying model_scale.")
                cfg_for_build = deepcopy(cfg_for_build)
                scales = cfg_for_build.get("scales")
                if not scales:
                    raise ValueError("model_scale/scale was set, but the model YAML does not define `scales:`.")
                if model_scale not in scales:
                    raise ValueError(f"Invalid model_scale/scale='{model_scale}'. Valid keys are {list(scales)}.")
                yaml_scale = cfg_for_build.get("scale")
                if yaml_scale and yaml_scale != model_scale:
                    LOGGER.info(f"Overriding model.yaml scale='{yaml_scale}' with model_scale='{model_scale}'.")
                cfg_for_build["scale"] = model_scale

            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=cfg_for_build)
            self.model = self.trainer.model

        self.trainer.hub_session = self.session  # attach optional HUB session
        self.trainer.train()
        # Cleanup: explicitly clear registered multimodal hooks after training ends
        # This is not a graceful-degrade path; it only releases resources when hooks are no longer needed.
        try:
            mm_hm = getattr(self.model, 'mm_hook_manager', None)
            if mm_hm is not None and hasattr(mm_hm, 'clear'):
                mm_hm.clear()
        except Exception:
            pass
    ...
    def profile(
        ...
        """
        统一的模型性能剖析接口：构造符合（多模态/单模态）要求的随机输入，并在 `predict(..., profile=True)` 下输出逐层耗时/GFLOPs。

        参数:
            imgsz: 输入尺寸，int 或 [h, w]。
            batch: 批大小，默认 2。
            channels: 通道数，优先级最高；未指定时按 `modality` 与路由器推断。
            modality: 'auto' | 'Dual' | 'RGB' | 'X'。
            device: 运行设备；默认取模型参数所在设备。
            warmup: CUDA 预热轮数，默认 2；CPU 可忽略。
            dtype: 张量精度，默认 float32。

        返回:
            与 `self.model.predict(..., profile=True)` 一致的返回值。
        """
        # 1) 规范化尺寸
        if isinstance(imgsz, int):
            h, w = int(imgsz), int(imgsz)
        elif isinstance(imgsz, (list, tuple)) and len(imgsz) == 2:
            h, w = int(imgsz[0]), int(imgsz[1])
        else:
            raise ValueError("imgsz 必须为 int 或 [h, w] 两元素序列")
        if h <= 0 or w <= 0:
            raise ValueError("imgsz 高宽必须为正整数")

        # 2) 设备与日志
        dev = device or next(self.model.parameters()).device

        # 3) 多模态通道推断
        mod = modality  # 实际使用的模态
        c = channels
        mm_router = getattr(self.model, "mm_router", None) or getattr(self.model, "multimodal_router", None)
        if c is None:
            if mod == "Dual":
                if mm_router and hasattr(mm_router, "INPUT_SOURCES"):
                    c = int(mm_router.INPUT_SOURCES.get("Dual", 6))
                else:
                    raise ValueError("未检测到多模态路由器，无法自动推断 Dual 通道数；请显式传入 channels 或改用 modality='RGB'")
            elif mod == "X":
                if mm_router and hasattr(mm_router, "INPUT_SOURCES"):
                    c = int(mm_router.INPUT_SOURCES.get("X", 3))
                else:
                    raise ValueError("未检测到多模态路由器，无法自动推断 X 通道数；请显式传入 channels 或改用 modality='RGB'")
            elif mod == "RGB":
                c = 3
            elif mod == "auto":
                if mm_router and hasattr(mm_router, "INPUT_SOURCES"):
                ...
                raise ValueError("modality 仅支持 'auto'|'Dual'|'RGB'|'X'")
        else:
            c = int(c)
            if c <= 0:
        ...
        # 4) 构造输入
        inputs = torch.randn((int(batch), c, h, w), dtype=dtype, device=dev)

        # 5) 可选预热（CUDA）
        if dev.type == "cuda" and warmup > 0:
            for _ in range(int(warmup)):
                _ = self.model.predict(inputs, profile=False)
            torch.cuda.synchronize(device=dev)

        # 6) 正式剖析
        LOGGER.info(
            f"Profile start: imgsz=({h},{w}), batch={batch}, channels={c}, modality={mod}, device={dev.type}"
        )
        # 补充：输出单路与全路 GFLOPs 指标
        try:
            # 单路/架构级 GFLOPs（不感知运行时路由）
            arch_gflops = compute_model_gflops(self.model, imgsz=[h, w], modality=None, route_aware=False)

            # 全路/运行时 GFLOPs（显式以 Dual 通道作为输入，触发两路计算）
            mm_router = getattr(self.model, "mm_router", None) or getattr(self.model, "multimodal_router", None)
            dual_ch = None
            if mm_router is not None and hasattr(mm_router, "INPUT_SOURCES"):
                dual_ch = mm_router.INPUT_SOURCES.get("Dual", None)

            if dual_ch is not None:
                try:
                    import thop  # type: ignore
                except Exception as _e:
                    raise RuntimeError("缺少 'ultralytics-thop' 依赖，无法统计全路 GFLOPs") from _e

                mcopy = deepcopy(self.model)
                mcopy.eval()
                pdev = next(mcopy.parameters()).device
                im_dual = torch.empty((1, int(dual_ch), h, w), device=pdev)
                route_gflops = thop.profile(mcopy, inputs=[im_dual], verbose=False)[0] / 1e9 * 2
            else:
                # 无多模态配置时，“全路”与“单路”等价
                route_gflops = arch_gflops

            LOGGER.info(
                f"GFLOPs[单路/架构]={arch_gflops:.2f} | GFLOPs[全路/运行时]={route_gflops:.2f}"
            )
        except Exception as e:
            # 明确报告失败原因
            LOGGER.warning(f"GFLOPs 统计失败: {e}")
